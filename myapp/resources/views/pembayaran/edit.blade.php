...
<div class="flex min-h-[80vh]">
  ...
  <main class="flex-1 p-10">
    <div class="flex items-start justify-between">
      <h1 class="text-3xl font-semibold tracking-tight">Edit Pembayaran</h1>
    ...
    <form method="POST" action="{{ route('pembayaran.update', $pembayaran) }}" class="mt-6 space-y-6">
      @csrf @method('PUT')
      ...
      <div class="max-w-5xl">
        ...
        <select name="siswa_id"
          ...
          @foreach ($siswa as $s)
            <option value="{{ $s->id }}" @selected(old('siswa_id', $pembayaran->siswa_id)==$s->id)>
              {{ $s->nama }} — {{ $s->kelas }}
            </option>
          @endforeach
        </select>
      ...
      <div class="max-w-xl">
        ...
        <input type="date" name="tanggal_bayar" value="{{ old('tanggal_bayar', $pembayaran->tanggal_bayar?->format('Y-m-d')) }}"
               class="block w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
      ...
      <div class="max-w-xl">
        ...
        <input type="number" min="0" step="1" name="jumlah" value="{{ old('jumlah', $pembayaran->jumlah) }}"
               class="block w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
      ...
      <div class="max-w-xl">
        ...
        <select name="status"
                class="block w-full rounded-md border border-gray-300 px-4 py-2 focus:border-blue-500 focus:ring-1 focus:ring-blue-500" required>
          <option value="belum_lunas" @selected(old('status', $pembayaran->status)=='belum_lunas')>Belum Lunas</option>
